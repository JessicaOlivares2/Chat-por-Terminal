...
wss.on('connection', ws => {
    ...
    ws.send(JSON.stringify({ tipo: 'servidor', contenido: '¡Bienvenido al chat!' }));
    ...
    ws.on('message', message => {
        try {
            ...
            if (datos.tipo === 'nombreUsuario') {
                ...
                console.log(`[Servidor]: Cliente ${clientId} estableció nombre de usuario: ${datos.nombreUsuario}`);
                broadcast(`[Servidor]: El usuario "${datos.nombreUsuario}" se ha unido al chat.`, ws);
            ...
            console.error('[Servidor]: Error al procesar el mensaje del cliente:', error);
        }
    });
    ...
    ws.on('close', () => {
        const clienteInfo = clientes.get(ws);
        if (clienteInfo && clienteInfo.nombreUsuario) {
            console.log(`[Servidor]: Cliente desconectado - Nombre: ${clienteInfo.nombreUsuario}, ID: ${clientId}`);
            broadcast(`[Servidor]: El usuario "${clienteInfo.nombreUsuario}" ha salido del chat.`, ws);
        }
        clientes.delete(ws);
    });

    ws.on('error', error => {
        console.error(`[Servidor]: Error en el WebSocket del cliente ${clientId}: ${error}`);
        clientes.delete(ws);
    });
});
...
function broadcast(mensaje, senderSocket) {
    ...
    clientes.forEach((clienteInfo, clienteSocket) => {
        if (clienteInfo.id !== senderId && clienteSocket.readyState === 1) {
            clienteSocket.send(JSON.stringify({ tipo: 'mensaje', contenido: mensaje }));
        }
    });
}
...
setInterval(() => {
    sendServerMessage('[Servidor]: Recordatorio - ¡Este es un chat por terminal!');
}, 100000);

...
async function main() {
  ...
  try {
    ...
    ws.on("open", () => {
      ...
      ws.send(
        JSON.stringify({ tipo: "nombreUsuario", nombreUsuario: username })
      );
      rl.prompt();
    });
    ...
    rl.on("line", (message) => {
      ...
      if (ws.readyState === WebSocket.OPEN && message) {
        const mensajeAEnviar = JSON.stringify({
          tipo: "mensaje",
          contenido: message,
        });
        ...
        console.log(
          "[Cliente]: No se envió el mensaje (conexión cerrada o mensaje vacío)."
        );
      }
      rl.prompt();
    });
